...
## 5. 会话架构：流水线模拟器 (Pipeline Simulator)

除了底层的鼠标键盘物理模拟外，**顶层会话流（Session Flow）的编排同样是防封号风控的核心**。

### 5.1 无状态调用的致命缺陷
早期的架构设计将无头浏览器当作“执行单次抓取/提交的 API 工具”，例如：
1. 启动浏览器 -> 搜索提取状态 -> 关闭浏览器
2. 启动浏览器 -> 直达帖子 URL 查看评论 -> 关闭浏览器
3. 启动浏览器 -> 直达帖子 URL 发送评论 -> 关闭浏览器

这种**碎片化、频繁重启浏览器、高频直达底层 URL (`page.goto`)** 的行为特征，完全违背了人类上网的常理，极易被小红书的安全策略标记为异常流量，从而频繁弹出扫码或滑块验证。

### 5.2 全新范式：沉浸式流水线
为了实现彻底的拟人化，系统业务层已升级为**“沉浸式流水线模拟器”**模式。在一个长生命周期的 `with XhsClient(...) as pipeline:` 会话中，不间断地完成一套连贯的物理动作：

1. **DOM 寻址物理导航 (Zero hard `goto`)**：真实人类不会直接在地址栏粘贴目标帖子 URL。流水线通过在搜索推荐页或主页寻找包裹目标 `note_id` 的卡片，通过触发物理点击 (`click_into_note_card`) 进入详情页。
2. **无感拦截与滚动阅读 (XHR Interception)**：通过 `page.on("response", ...)` 静默监听 `/api/sns/web/v2/comment/page` 接口。结合模拟鼠标滚轮向下滚动（`mouse.wheel`）和随机阅读停顿，系统能在不解析复杂 DOM 的情况下，安全无感地获取数百条动态加载的评论，彻底突破了初期仅靠解析 `__INITIAL_STATE__` 只能拿到前 20 条评论的瓶颈。
3. **基于视觉文本的鲁棒互动 (Visual Targeting)**：在决定回复某条评论时，彻底抛弃了对前端内部 `[id="..."]` 等脆弱 DOM 属性的依赖。转而利用 Playwright 的 `.filter(has_text=目标评论文本)`，在“人类视觉可见”的层面锁定评论块，再在其内部寻找并点击“回复”按钮。这完全映射了人类“用眼睛寻找文本 -> 移动鼠标点击对应回复区”的认知过程。

## 6. 舍弃的替代方案 (Alternatives Considered)
...
通过**“驱动层做物理模拟（`cloakbrowser` 拦截）”** + **“会话层做流水线编排（Pipeline Simulator 连贯轨迹）”** 的双轨协同策略，XHS CLI 获得了极其强悍的反指纹自动化能力。这在确保代码可读性与易维护性的同时，最大化地在生物学与行为逻辑学上模拟了一个真实的“小红书深度用户”，实现了长期稳定的无感自动化交互。
